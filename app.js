// ==================== LÓGICA DE AUTENTICACIÓN A PRUEBA DE FALLOS ====================
let currentUser = "";
let memoryDB = {
...
function loadProfileData() {
    ...
    document.getElementById('prof-doc-type').value = data.docType || "";
    document.getElementById('prof-doc-num').value = data.docNum || "";
    ...
    document.getElementById('prof-eps').value = data.eps || "";
    document.getElementById('prof-arl').value = data.arl || "";
    
    // Antecedentes Clínicos
    document.getElementById('prof-conditions').value = data.conditions || "";
    document.getElementById('prof-meds').value = data.meds || "";
    document.getElementById('prof-history').value = data.history || "";
}

function enableProfileEdit(enable) {
    const fields = [
        'prof-name', 'prof-doc-type', 'prof-doc-num', 'prof-dob', 'prof-gender', 'prof-phone', 'prof-address',
        'prof-blood', 'prof-eps', 'prof-arl',
        'prof-conditions', 'prof-meds', 'prof-history'
    ];
...
function handleSaveProfile(e) {
    ...
    users[currentUser].docType = document.getElementById('prof-doc-type').value;
    users[currentUser].docNum = document.getElementById('prof-doc-num').value;
    users[currentUser].dob = document.getElementById('prof-dob').value;
    users[currentUser].gender = document.getElementById('prof-gender').value;
    users[currentUser].phone = document.getElementById('prof-phone').value;
    users[currentUser].address = document.getElementById('prof-address').value;
    users[currentUser].blood = document.getElementById('prof-blood').value;
    users[currentUser].eps = document.getElementById('prof-eps').value;
    users[currentUser].arl = document.getElementById('prof-arl').value;
    users[currentUser].conditions = document.getElementById('prof-conditions').value;
    users[currentUser].meds = document.getElementById('prof-meds').value;
    users[currentUser].history = document.getElementById('prof-history').value;
    ...
    alert("¡Ficha médica guardada exitosamente! Tu información está protegida.");
    showView('dashboard-view');
}
...
function showView(viewId) {
    ...
    // Control visual de colores según estado de emergencia
...
function activateSelectedState() {
    ...
    if(activeIdx === 0) {
        ...
        addLog("Emergencia Crítica", "S.O.S Rojo manual."); incrementCounter("danger");
        showActiveStateScreen();
    }
}
...
function showActiveStateScreen() {
    ...
    if (activeIdx === 0) {
        // MONITOREO DE SALUD EN ESTADO SEGURO
        ...
        // RADAR Y GUÍA EN EMERGENCIA / ALERTA
        contentZone.innerHTML = `
...
function simulateAlexaCommand(command) {
    ...
    if(command === 'emergencia') {
        activeIdx = 2; viewIdx = 2; renderCarousel(); showActiveStateScreen();
        addLog("Emergencia Crítica", "Activación por comando de voz (Alexa)."); incrementCounter("danger");
    } else if(command === 'alerta') {
        activeIdx = 1; viewIdx = 1; renderCarousel(); showActiveStateScreen();
        addLog("Alerta Preventiva", "Activación por comando de voz (Alexa)."); incrementCounter("warning");
    } else if(command === 'cancelar') {
        if(activeIdx === 0) return alert("El sistema ya está seguro.");
        activeIdx = 0; viewIdx = 0; renderCarousel(); showView('dashboard-view');
        addLog("Cancelación de Alerta", "Cancelación por voz (Alexa)."); incrementCounter("cancel");
    }
}

// ==================== CANCELAR Y REGISTROS ====================
...
// ==================== CONTACTOS ====================
let myContacts = [{ id: 1, name: "Papá", phone: "311 000 0000" }];
...
// =========================================================================
// ================= LÓGICA DEL PANEL DE ACCESIBILIDAD (UX/US) =============
// =========================================================================
...
function updateA11y() {
    ...
    const fontFace = document.getElementById('a11y-font-face').value;
    const fontSize = document.getElementById('a11y-font-size').value;
    const letterSpacing = document.getElementById('a11y-letter-spacing').value;
    const lineHeight = document.getElementById('a11y-line-height').value;
    const pWidth = document.getElementById('a11y-p-width').value;
    const textAlign = document.getElementById('a11y-text-align').value;
    const txtColor = document.getElementById('a11y-txt-color').value;
    const bgColor = document.getElementById('a11y-bg-color').value;
    
    // Actualizar etiquetas visuales de los sliders
    document.getElementById('val-font-size').innerText = fontSize + '%';
    document.getElementById('val-letter-spacing').innerText = letterSpacing + 'px';
    document.getElementById('val-line-height').innerText = lineHeight;
    document.getElementById('val-p-width').innerText = pWidth + '%';

    // Aplicar a Variables CSS y tamaño base (Rem)
    root.style.setProperty('font-size', fontSize + '%'); // Escala toda la página matemáticamente
    root.style.setProperty('--a11y-font-family', fontFace);
    root.style.setProperty('--a11y-letter-spacing', letterSpacing + 'px');
    root.style.setProperty('--a11y-line-height', lineHeight);
    root.style.setProperty('--a11y-p-width', pWidth + '%');
    root.style.setProperty('--a11y-text-align', textAlign);
    root.style.setProperty('--a11y-text-color', txtColor);
    root.style.setProperty('--a11y-bg-color', bgColor);

    // Toggles de Visibilidad (Checks)
    const highlightLinks = document.getElementById('a11y-link-highlight').checked;
    const hideImages = document.getElementById('a11y-image-visibility').checked;
...
function resetA11y() {
    ...
    document.body.classList.remove('a11y-active', 'a11y-highlight-links', 'a11y-hide-images');
    
    // Resetear tamaño de fuente raíz a por defecto
    document.documentElement.style.removeProperty('font-size');
    
    // Limpiar formulario y resetear etiquetas visuales
    document.getElementById('a11y-form').reset();
    document.getElementById('val-font-size').innerText = '100%';
    document.getElementById('val-letter-spacing').innerText = '0px';
    document.getElementById('val-line-height').innerText = '1.5';
    document.getElementById('val-p-width').innerText = '100%';
    
    // Si la app estaba en un estado particular (ej: emergencia roja), asegurar que retome su color base original
    applyActiveState(); 
}